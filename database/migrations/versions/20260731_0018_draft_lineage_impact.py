"""Record bounded impact times for PostgreSQL draft lineage changes.
...
def upgrade() -> None:
    op.execute(
        text(
            """
            CREATE FUNCTION draft_lineage_match_impact(target_match_id bigint)
            RETURNS bigint AS $$
                SELECT MIN(candidate.affected_from)
                  FROM (
                      SELECT match.start_time AS affected_from
                        FROM matches AS match
                       WHERE match.match_id=target_match_id
                         AND match.start_time>0
                  ) AS candidate
                 WHERE candidate.affected_from>0;
            $$ LANGUAGE sql STABLE
            """
        )
    )
...
def downgrade() -> None:
    ...
    op.execute(text("DROP FUNCTION draft_lineage_match_impact(bigint)"))
