...
function isPrematchProviderStatus(value: string): boolean {
  return PREMATCH_PROVIDER_STATUSES.has(value.trim().toLowerCase());
}
...
export function MatchWorkspace({
  ...
  const prematchWinner = !liveWinner && isPrematchProviderStatus(match.provider_status)
...
function CurrentStrategyOverview({
  ...
  const scanText = strategy.data
    ...
    : "尚无可信策略输出";
  const prematchWinner = isPrematchProviderStatus(match.provider_status)
    ? detail?.prematch_winner || null
    : null;
