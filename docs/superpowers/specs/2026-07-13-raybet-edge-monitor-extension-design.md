...
- Authenticated delivery to a companion bound to `127.0.0.1`.
...
- A small popup for capture health and an options page for local pairing.
...
```text
C:\Users\59908\dota2-ad-assistant\edge-extension
```

It owns browser hooks, sanitization, buffering, pairing UI, and delivery. It
...
It owns authentication, schema validation, deduplication, audit persistence,
...
`dota2-ad-assistant` is not currently a Git repository. The implementation
plan must therefore treat extension version-control setup as an explicit
delivery decision rather than silently initializing a repository or mixing the
extension into the predictor repository.
...
- companion reachability and authentication state
...
The options page is limited to:

- one-time companion pairing
- enabled domains within the manifest's fixed RayBet host allowlist
- debug level: `off`, `errors`, or `diagnostic`
...
runtime-configurable in version 1. The companion must refuse startup with a
wildcard or non-loopback bind address.

On first start, it generates a one-time pairing code, prints it locally, and
expires it after ten minutes or one successful use. The options page sends the
code to `POST /v1/pair`. A valid pairing returns a random 256-bit shared secret
and records the calling extension origin. Pairing is rate-limited to five
attempts per minute and failure responses do not reveal whether a code prefix
was correct.

The extension persists only the shared secret, pairing protocol state, enabled
domain flags, and debug level in `chrome.storage.local`. Event bodies, event
hashes, counters, and queues are never persisted there. The companion stores
its secret outside the Git repository under the current user's local
application data directory with user-only permissions where supported.
Secrets are never written to logs, SQLite, fixtures, reports, or Git.
...
The signature is HMAC-SHA256 over timestamp, nonce, HTTP method, path, and body
SHA-256. The companion rejects timestamps outside a 30-second window, reused
nonces, an extension origin different from the paired origin, invalid content
type, and bodies above 1 MiB. Nonces are retained in a bounded five-minute
replay cache.
...
Consumes the active one-time code and establishes the shared secret. Disabled
after successful pairing until the user explicitly resets pairing from the
local command line.

### `POST /v1/events`

Accepts an authenticated array of 1 to 50 schema-versioned events. The response
returns accepted, duplicate, and rejected event IDs with stable reason codes.
The entire body must be valid JSON and within 1 MiB. Individual invalid events
are rejected without preventing valid events in the same authenticated batch
from being persisted.

### `GET /v1/status`

Returns authenticated operational state: latest accepted timestamp, event type
...
All non-health endpoints enforce authentication and per-origin rate limits.
CORS allows only the exact extension origin recorded during pairing. The
pairing endpoint temporarily accepts a `chrome-extension` origin only when
accompanied by the active one-time code.
...
- `error`: manifest, storage, pairing, or validation failure requires attention.
...
- Authentication failure stops delivery until re-paired; it does not repeatedly
  brute-force or flood the companion.
...
- Pairing expiry, one-use semantics, origin binding, and rate limiting.
- HMAC validation, timestamp window, nonce replay rejection, and body limits.
...
- companion batches authenticate and deduplicate across retries
...
- size filtering
- queue overflow
- delivery/authentication failure
...
4. Add the companion schema, pairing/authentication, API tests, and
...
- The companion listens only on `127.0.0.1`, requires one-time pairing, rejects
  replayed or unauthenticated batches, and never exposes payloads through its
  status API.
