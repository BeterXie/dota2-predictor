...
The companion listens only on localhost and retains the existing pairing/HMAC
contract. Extension domain allowlists, pause state, payload size limits, and
sanitization remain enforced. Logs never include raw credentials, query tokens,
authorization codes, or unsanitized browser payloads.

Deployment requires:

1. Manually loading `C:\Users\59908\dota2-ad-assistant\edge-extension` from
   `edge://extensions` in Developer mode.
2. Pairing the extension with the localhost companion.
3. Configuring the sender address and SMTP authorization code locally without
